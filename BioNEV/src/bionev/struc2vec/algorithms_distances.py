...
def getCompactDegreeListsVertices(g, vertices, maxDegree, calcUntilLayer):
    ...
    degreeList[vertices] = getCompactDegreeLists(g, vertices, maxDegree, calcUntilLayer)
...
def splitDegreeList(part, c, G, compactDegree):
    ...
    # for v in c:
    nbs = get_vertices(c, len(G[c]), degrees, a_vertices)
    vertices[c] = nbs
    degreeListsSelected[c] = degreeList[c]
    for n in nbs:
...
def exec_bfs_compact(G, workers, calcUntilLayer):
    ...
    parts = workers
    chunks = np.partition(vertices, parts)
