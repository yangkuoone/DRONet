...
def read_for_gae(filename, weighted=False):
    ...
    edgelist = np.loadtxt(filename,dtype='str')
    if weighted:
        edgelist = [(str(edgelist[idx, 0]), str(edgelist[idx, 1])) for idx in range(edgelist.shape[0]) if
                    edgelist[idx, 2] > 0]
    else:
        edgelist = [(str(edgelist[idx, 0]), str(edgelist[idx, 1])) for idx in range(edgelist.shape[0])]
