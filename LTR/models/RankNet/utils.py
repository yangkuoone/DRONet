...
def load_train_vali_data(data_fold, small_dataset=False):
    """
    :param data_fold: str, which fold's Fold1 was going to use to train
    :return:
    """
    if small_dataset:
...
def eval_ndcg_at_k(
    ...
    hit1=0
    mrr=0
    example_file=open("./resultdata/RankNet/example_rankNet_ran_1v1.txt",'w',encoding='utf8')
    ...
    for qid in result_df.qid.unique():
        ...
        h1=0
        mr=0
        mydf=result_df[result_df.qid == qid]
        mylabel=[]
        scoredic={}
        for i,v in enumerate(mydf.values):
            scoredic[i]=v[2]
            if v[1]>0:
                mylabel.append(i)
        scoredic=sorted(scoredic.items(), key=lambda item:item[1], reverse=True)
        example=list(dict(scoredic).keys())[:10]
        lst=[]
        for e in example:
            lst.append(str(e))
        example_file.write(str(int(qid))+"\t"+",".join(lst)+"\n")
        for v in (mylabel):
            ylist = list(dict(scoredic).keys())
