...
#############################
def train_rank_net(
    start_epoch=0, additional_epoch=300, lr=0.0005, optim="adam",
    ...
    # get training and validation Fold1:
    data_fold = 'Fold1'
    ...
    losses = []
    # result = open("/home/yyx/pycharm_project/LTR/0819data/ranknet_200_1v5_1v5.txt", 'w', encoding='utf8')
    # result_loss = open("/home/yyx/pycharm_project/LTR/0819data/ranknet_200_1v5_1v5_loss.txt", 'w', encoding='utf8')
    for i in range(start_epoch, start_epoch + additional_epoch):
        ...
        # result_loss.write(str(i)+"\t"+str(epoch_loss)+"\n")

        # save to checkpoint every 5 step, and run eval
        if i % 1 == 0 and i != start_epoch:
            save_to_ckpt(ckptfile, i, net, optimizer, scheduler)
            net_inference.load_state_dict(net.state_dict())
            ndcg_result,mymetric=eval_model(net_inference, device, df_valid, valid_loader, i, writer)
            # result.write(str(i)+"\t"+str(ndcg_result[1])+"\t"+str(ndcg_result[3])+"\t"+str(ndcg_result[5])+"\t"+str(ndcg_result[10])+"\t"+str(ndcg_result[30])+"\n")
            # result.write(str(i)+"\t"+str(ndcg_result[10])+"\t"+str(mymetric[0])+"\t"+str(mymetric[1])+"\t"+str(mymetric[2])+"\t"+str(mymetric[3])+"\n")
    # save the last ckpt
    save_to_ckpt(ckptfile, start_epoch + additional_epoch, net, optimizer, scheduler)

    # final evaluation
    net_inference.load_state_dict(net.state_dict())
    ndcg_result,mymetric = eval_model(
...
def eval_model(inference_model, device, df_valid, valid_loader, epoch, writer=None):
    ...
    with torch.no_grad():
        ...
        ndcg_result , mymetric= eval_ndcg_at_k(
            inference_model, device, df_valid, valid_loader, batch_size, [1,3,5,10,30], epoch, writer)
    return ndcg_result,mymetric
